...
\title{Aggregation and Interpolation in CFD--DEM Based on Diffusion Equation}
...
\begin{align}
\frac{\partial u}{\partial t} & = \frac{\partial^2 u}{\partial x^2} \textrm{  on  } -\infty < x < \infty , t > 0 \\
	u & = f(x)  \textrm{  at  } t = 0
\end{align}
...
\begin{align}
u(x, t) & = \int_{-\infty}^{\infty} K(x-\xi)  \; \delta(\xi - x_0) d \xi \\
	& = K(x-x_0, t)
\end{align}
...
\begin{align}
u(x, t) = \sum_{i=1}^{N_c} c_i  \; K(x - x_i, t)
\end{align}

We are interested in the solution at a fixed time $\tau$, denoted as $\hat{u}(x) \equiv u(x, \tau)$. The solution is rewritten as: 
\begin{equation}
\hat{u}(x) = \sum_{i=1}^{N_c} c_i  \; K(x - x_i) 
...
One can interpret $c_i$ as the total solid particle volume in cell $i$.  Correspondingly, $\hat{u}(x)$ is the solid volume fraction at any location $x$. 
\begin{equation}
\hat{u} (x) = \left. \frac{dV_s}{dV}  \right|_{x}
\end{equation}
...
\begin{enumerate}
\item Comparison of  kernel function and diffusion equation based aggregation with 3 to 5 particles on 1D and 2D domains. Demonstrate the equivalence between the two methods.
\item On a 2D Cartesian grid, perform volume fraction aggregation and compare with analytical and two-grid formulations.
\item Aggregation of volume fraction on a stretched grid, and on a unstructured grid. 
\item Test fluidization case with PCM and diffusion-based aggregation, and compare pressure signal. (Ref to AIChe paper).
\end{enumerate}
